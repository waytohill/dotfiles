...
local options = require("config.terminal.options")
...
for k, v in pairs(require("config.terminal.theme-setup.theme-overrides")) do
...
for _, tweak in ipairs(options.TWEAKS) do
  local ok, error = pcall(require, "config.terminal.tweaks." .. tweak)
  if not ok then
...
for _, tweak in ipairs(options.LANGUAGES) do
  local ok, error = pcall(require, "config.terminal.languages." .. tweak)
  if not ok then
...
if vim.g.neovide then
  require("config.terminal.gui.neovide")
end

-- Firenvim Setup
if vim.g.started_by_firenvim then
  require("config.terminal.gui.firenvim")
end
